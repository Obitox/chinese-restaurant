...
// AddItem adds a new item to the DB
func AddItem(w http.ResponseWriter, r *http.Request) {
	...
	unmarshErr := json.Unmarshal(data, &item)
	if unmarshErr != nil {
	...
	log.Println("ITEM: " + item.RequestAntiForgeryToken)
	log.Println("COOKIE: " + csrfCookie.Value)

	if len(item.RequestAntiForgeryToken) > 0 {
		if item.RequestAntiForgeryToken == csrfCookie.Value {
